...
- 2.1.0：优化了AI的能力，减少了token的消耗，提高了AI的查文档和总结的能力。当AI通过WIKI百科总结的时候会给出提示
- 2.0.0：重构为 `supervisor-agent` + `execute-agent` 双 Agent 编排。
- 2.0.0：增强 wiki 标题候选评分、在线标题兜底、候选页摘要筛选和证据校验，提高关键词命中率和总结准确性。
- 2.0.0：修复背包容量表述，改为“已用 N 格，容量 M 格”；优化农场作物读取和失败原因说明。
- 优化了Agent编排，回答问题更精准
...
- 内置本地 wiki 标题关键词本。Agent 会先查询标题候选，再决定读取哪些真实页面，支持从关键词到相关页面的多跳检索。
- 支持 wiki 重定向跟随，例如查询旧译名、别名或英文名时，会继续读取目标页面内容。
- wiki 页面会先清洗为文本全文交给 execute-agent；最终复核只传递答案实际使用的证据片段。
- 如果答案使用了 wiki 查询结果，回复末尾会标记 `（本答案通过wiki总结得出）`。
...
6. 点击右上角 `清空` 清除当前会话历史。
7. 输入框为空时可再次按 `L` 关闭面板；输入框有内容时可按 `Escape` 关闭。

## Agent 工作流程

Mod 会按双 Agent 编排处理问题，智能优先：

1. `supervisor-agent` 负责 plan / replan，判断是否需要查询 wiki，并输出短查询词和需要验证的事实。
2. 工具层用本地 `Data/wiki-titles.json` 查询真实 wiki 标题候选，不在代码里硬编码具体游戏知识映射。
3. `execute-agent` 根据标题候选选择要读取的页面，再读取 biligame wiki 清洗后的全文。
4. `execute-agent` 基于当前游戏状态和 wiki 全文生成候选答案，并抽取本次答案实际使用到的证据片段。
5. `supervisor-agent` 只接收候选答案、证据片段、已查询页面和当前游戏状态进行最终检查，不再重复接收 wiki 全文。
6. 如果证据不足，`supervisor-agent` 最多 replan 一次，由 `execute-agent` 再查询和回答。

这意味着一次复杂提问可能会触发多次 AI 请求，速度会比单次问答慢，但规划、检索和推理能力更强。(建议使用 Flash 模型，回复速度还是很快的)
...
数值越大，execute-agent 可参考的页面越多，但响应可能更慢、token 消耗更高。
...
- `wiki-title-search`：Agent 查询词对应的本地 wiki 标题候选。
- `wiki-candidate-preview`：候选页面摘要读取情况，用于帮助 execute-agent 决定读取哪些全文页面。
- `wiki-page-fetch`：真正读取的 wiki 页面、文本长度和缓存命中情况。
- `execute-full-wiki-input`：execute-agent 收到的 wiki 全文页面数量和字符数。
- `execute-evidence`：execute-agent 从全文中抽取的答案证据片段。
- `supervisor-review-input`：supervisor-agent 最终复核输入，只包含候选答案和证据片段，不包含 wiki 全文。

注意：2.0.0 后 supervisor-agent 复核阶段不再重复读取 wiki 全文；真实页面读取次数以 `wiki-page-fetch` 为准。

wiki 内容限制：

- 抓取阶段会保存清洗后的页面全文，最多约 `60000` 字。
- `execute-agent` 可以读取较长 wiki 全文。
- `supervisor-agent` 复核时只读取 execute-agent 抽取的证据片段。
- 如果发生截断，日志会出现类似 `...[truncated execute wiki input ...]` 或 `...[truncated evidence ...]` 的标记。
...
- 农场动物摘要
- 已学烹饪配方、已学制作配方数量
- 出货、钓鱼、矿物、古物、已做料理等收集统计摘要
- Joja 会员、社区中心、温室、矿车、巴士、桥梁、淘金盘、威利的船、姜岛等关键进度标记
