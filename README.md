...
把整个 `StardewAiAssistant` 压缩包解压到到游戏的 `Mods` 目录。
...
3. `execute-agent` 按计划调用 wiki 工具查询页面，并再次调用 AI 判断工具结果是否足够支持最终答案。
4. 如果结果不足，`plan-agent` 会执行 replan，再次调用 AI 根据失败日志生成新的关键词并重新查询。
5. `supervisor-agent` 最后再次调用 AI，汇总当前游戏状态、wiki 结果、执行日志和最近对话，输出最终答案。
...
- 如果 AI 回答明显不准确，可以开启 `EnableDebugLogging` 并查看 SMAPI 日志。
